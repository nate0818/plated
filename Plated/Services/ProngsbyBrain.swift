...
struct ProngsbyBrain {
    ...
    private func scaleRecipe(_ text: String) -> String? {
        ...
        guard let recipe = recipeMatch(in: text) else {
            return "Tell me which dish and how many. Try \"scale \(recipes.first?.title ?? "Pizza Night") for 8\" and I'll redo the math so you don't have to."
        }
        var target: Int?
        ...
        return opener() + "\(recipe.title), rescaled from \(recipe.servings) to \(target):\n\n\(lines.joined(separator: "\n"))\n\nCook times mostly hold, so long as you don't crowd the pan. Crowded pans steam, and steamed is nobody's favorite adjective."
    }
