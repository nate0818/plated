...
final class GalleryTests: XCTestCase {
    ...
    func testNativeScreens() throws {
        ...
        tap("By meal")
        let mealSelector = app.buttons["grocery-meal-selector"]
        XCTAssertTrue(mealSelector.waitForExistence(timeout: 5))
        mealSelector.tap()
    ...
    func testServingPlan() throws {
        ...
        Thread.sleep(forTimeInterval: 1)
        let recipePlan = app.buttons["recipe-plan"]
        XCTAssertTrue(recipePlan.waitForExistence(timeout: 5))
        XCTAssertTrue(recipePlan.isHittable)
        recipePlan.tap()
    ...
    func testTableNotificationsBell() throws {
        ...
        XCTAssertTrue(app.buttons.matching(identifier: "Account").allElementsBoundByIndex.contains { $0.isHittable })
        ...
        back.tap()
        let returnedBell = app.buttons["table-notifications-bell"]
        XCTAssertTrue(returnedBell.waitForExistence(timeout: 5))
        XCTAssertEqual(returnedBell.value as? String, "No unread notifications")
        tap("My Table")
    ...
    func testRecipeNativeDrag() throws {
        ...
        editorSteps()
        XCTAssertEqual(fieldStep(1).value as? String, "Heat the pan.")
        stepHandle(3).press(forDuration: 1.0, thenDragTo: stepHandle(1))
        Thread.sleep(forTimeInterval: 1)
    ...
    func testWeekCardNativeDrag() throws {
        ...
        let todayButton = app.buttons[weekDateIdentifier(todayDate)]
        let tomorrowButton = app.buttons[weekDateIdentifier(tomorrowDate)]
        let card = app.buttons["featured-dinner-card"]
        XCTAssertTrue(card.waitForExistence(timeout: 5))
        card.press(forDuration: 1.0, thenDragTo: tomorrowButton)
        Thread.sleep(forTimeInterval: 1)
        try shot("23-featured-after-drag")
        XCTAssertTrue(tomorrowButton.isSelected)
        todayButton.tap()
        XCTAssertEqual(card.label, "Second dinner")
        let source = app.descendants(matching: .any).matching(identifier: weekMealIdentifier(todayDate)).firstMatch
        let target = app.descendants(matching: .any).matching(identifier: weekMealIdentifier(tomorrowDate)).firstMatch
        ...
        for _ in 0..<6 { if todayButton.isHittable { break }; app.swipeDown() }
        todayButton.tap()
        XCTAssertEqual(card.label, "Drag test dinner")
    }

    func weekDateIdentifier(_ date: Date) -> String {
        let components = Calendar.current.dateComponents([.year, .month, .day], from: date)
        return "week-date-\(components.year ?? 0)-\(components.month ?? 0)-\(components.day ?? 0)"
    }

    func weekMealIdentifier(_ date: Date) -> String {
        let components = Calendar.current.dateComponents([.year, .month, .day], from: date)
    ...
    func testAccountSettingsAccessibilityLayout() throws {
        ...
        XCTAssertNotNil(account)
        account?.tap()
        XCTAssertTrue(app.staticTexts["Your place at the table."].waitForExistence(timeout: 5))
        XCTAssertTrue(app.buttons["account-edit-profile"].exists)
        let profileAction = app.buttons["account-view-your-table-profile"]
        XCTAssertTrue(profileAction.exists)
        XCTAssertLessThanOrEqual(profileAction.frame.height, 60, "Profile action must remain a single line")
        ...
        app.buttons["account-settings"].tap()
        XCTAssertTrue(app.staticTexts["Make Plated yours."].waitForExistence(timeout: 5))
